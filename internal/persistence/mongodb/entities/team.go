...
func FromDomainTeam(m *models.Team) *TeamEntity {
    ...
    for _, mm := range m.Members {
        ...
        if mm.UserID != "" {
            if oid, err := primitive.ObjectIDFromHex(mm.UserID); err == nil {
                userID = oid
            }
        }
        members = append(members, TeamMemberEntity{UserID: userID, Role: mm.Role, JoinedAt: mm.JoinedAt})
    }
    return &TeamEntity{
        ID:          m.ID,
        Name:        m.Name,
...
func (e *TeamEntity) ToDomainTeam() *models.Team {
    ...
    for _, mm := range e.Members {
        members = append(members, models.TeamMember{UserID: mm.UserID.Hex(), Role: mm.Role, JoinedAt: mm.JoinedAt})
    }
    return &models.Team{
        ID:          e.ID,
        Name:        e.Name,
